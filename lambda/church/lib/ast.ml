...
let t_add = Abs("m",Abs("n", (* take two numbers *)
  (* return a function that takes a function s and applies it
     m+n times to z*)
  Abs("s",Abs("z",
    App(Var "m", App(Var "s", (* apply s to n m times *)
      App(Var "n",App(Var "s",Var "z")) (* apply s to z n times *)
    ))
  ))
))
...
let s_add = "fun m. fun n. fun s. fun z. m s (n s z)"
